...
#pragma omp single nowait
            ...
            do {
                if (local_bin_index < local_bins.size()) {
                    size_t copy_start = fetch_and_add(next_frontier_tail,
                                                      local_bins[local_bin_index].size());
                    copy(local_bins[local_bin_index].begin(),
                         local_bins[local_bin_index].end(), frontier.data() + copy_start);
                    local_bins[local_bin_index].resize(0);
                    local_bin_index++;

                } else break;
                //int cur = fetch_and_add(binSize,local_bins[local_bin_index].size());
                //if (local_bin_index != next_bin_index+1) cout<<local_bin_index<<' '<<next_bin_index<<' '<<frontier.size()<<' '<<cur<<endl;
